...
class MongoStore {
	...
	constructor(options) {
		...
		if (!allOptions.collection && !allOptions.client && (!allOptions.collectionName || !allOptions.uri)) {
			throw new Error('collection/client or collectionName and uri should be set');
		}
	 ...
	 */
	init(options) {
		this.expireTimeMs = _.get(options, 'windowMs', this.expireTimeMs);
	}
